...
var DeleteFeature = OpenLayers.Class(OpenLayers.Control, {
	...
	setMap : function(EnterpriseMap) {
		this.handler.setMap(EnterpriseMap);
...
function ResizeMap(lon, lat) {
	EnterpriseMap.setCenter(new OpenLayers.LonLat(lon, lat).transform(WGS84,
			WGS84Google));
}
...
function init() {
	...
	// Initialize the EnterpriseMap
	// creates a new openlayers EnterpriseMap in the <div> html element with
	// id="EnterpriseMap"
	EnterpriseMap = new OpenLayers.Map("basicMap", {
	...
	editableLayer.events.register("beforefeatureremoved", '', function(obj) {
		if (obj.feature.popup) {
			EnterpriseMap.removePopup(obj.feature.popup);
		}
	});
	...
	EnterpriseMap.addLayers([ openstreetmap, editableLayer ]);
	// EnterpriseMap.zoomToExtent(mapextent);
	EnterpriseMap.zoomTo(9);
	...
	EnterpriseMap.addControl(selectControl);
	...
	EnterpriseMap.addControl(boxSelect);
	boxSelect.activate();
	...
	EnterpriseMap.addControl(panel);
...
function extend() {
	...
	if (ex) EnterpriseMap.zoomToExtent(ex);
}